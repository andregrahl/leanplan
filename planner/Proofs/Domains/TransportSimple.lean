...
/-
Transport, simple heuristic: goal-aware, consistent, admissible.

`drop` adds one `at` atom for a package and `drive` one for a vehicle; no
schema adds two.
...
theorem simple_goalAware (t : Task) (hwf : Task.WF t)
  ...
  (t.maxMissingOf_admissible hwf "transport-simple" [["at"]] hcert).1

theorem simple_consistent (t : Task) (hwf : Task.WF t)
    (hcert : certified t = true) : t.Consistent (simple t).eval :=
  (t.maxMissingOf_admissible hwf "transport-simple" [["at"]] hcert).2.1
...
theorem simple_admissible (t : Task) (hwf : Task.WF t)
  ...
  (t.maxMissingOf_admissible hwf "transport-simple" [["at"]] hcert).2.2
...
theorem simple_goalAware_of_schema (t : Task) (hwf : Task.WF t)
  ...
  (t.maxMissingOf_admissible_of_schema hwf "transport-simple"
    [["at"]] hgnd hsafe).1
